...
int main(int argc, char** argv) {
	...
	for (int i = 0; i < 128; i++)
	{
		cos_samples[i] = std::cos(2.0 * M_PI * 8.0 * (float)(i * step)) * std::pow(-1, i);
		outCosFile << cos_samples[i].real() << std::endl;
	}
	outCosFile.close();
